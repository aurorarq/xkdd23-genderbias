...
# This script uses the employee promotion dataset and the "full" classification strategy described in the paper.
...
df[gender_feat].replace(['f','m'],[0,1],inplace=True)

# For other features with more than 2 categories, use one-hot-encoding
df_non_num = df.select_dtypes(exclude=['int', 'float', 'bool'])
col_names = df_non_num.columns.values
...
# Performance metrics
cf_matrix = confusion_matrix(y_test_all, y_pred_rf_all)
ml.save_confusion_matrix_text(cf_matrix, filename="../results/metrics/promotion/rf_all_metrics.txt")

# Fairness metrics

fmetric_by_group, fmetric_dif, fmetric_overall, eod = metrics.evaluate_fairness_metrics(y_test_all, y_pred_rf_all, df_test_all, gender_feat)
...
# FP stats by gender
num_fp_fem, perc_fp_fem, num_fp_male, perc_fp_male = stats.fp_stats_by_gender(df_test_all, y_test_all, y_pred_rf_all, gender_feat)
...
# FN stats by gender
num_fn_fem, perc_fn_fem, num_fn_male, perc_fn_male = stats.fn_stats_by_gender(df_test_all, y_test_all, y_pred_rf_all, gender_feat)
...
cf_matrix = confusion_matrix(y_test_all, y_pred_gb_all)
ml.save_confusion_matrix_text(cf_matrix, filename="../results/metrics/promotion/gb_all_metrics.txt")

# Fairness metrics
fmetric_by_group, fmetric_dif, fmetric_overall, eod = metrics.evaluate_fairness_metrics(y_test_all, y_pred_gb_all, df_test_all, gender_feat)
...
# FP stats by gender
num_fp_fem, perc_fp_fem, num_fp_male, perc_fp_male = stats.fp_stats_by_gender(df_test_all, y_test_all, y_pred_gb_all, gender_feat)
print(f"FP (female): {num_fp_fem}, percentage (all female in test): {perc_fp_fem}")
print(f"FP (male): {num_fp_male}, percentage (all male in test): {perc_fp_male}")

# FN stats by gender
num_fn_fem, perc_fn_fem, num_fn_male, perc_fn_male = stats.fn_stats_by_gender(df_test_all, y_test_all, y_pred_gb_all, gender_feat)
print(f"FN (female): {num_fn_fem}, percentage (all female in test): {perc_fn_fem}")
print(f"FN (male): {num_fn_male}, percentage (all male in test): {perc_fn_male}")
...
clf_name = 'rf_all'
df_fp = ml.get_fp(df_test_all, y_test_all, y_pred_rf_all)
...
clusters_rf_all_fp_fem, exemplars_rf_all_fp_fem = explainer.cluster_affinity()
filename = folder + '/' + dataset_name + '_' + clf_name + '_fp_female_clustering.txt'
stats_fp.save_clustering_results(filename, df_fp_fem, clf_name, 'female', clusters_rf_all_fp_fem, exemplars_rf_all_fp_fem)

# Random Forest + FP + male
df_fp_male = df_fp[df_fp[gender_feat]==1]
explainer = clue.ClusterExplainer(df_fp_male.values)
clusters_rf_all_fp_male, exemplars_rf_all_fp_male = explainer.cluster_affinity()
filename = folder + '/' + dataset_name + '_' + clf_name + '_fp_male_clustering.txt'
stats_fp.save_clustering_results(filename, df_fp_male, clf_name, 'male', clusters_rf_all_fp_male, exemplars_rf_all_fp_male)

# Random Forest + FN + Female
df_fn = ml.get_fn(df_test_all, y_test_all, y_pred_rf_all)
...
clusters_rf_all_fn_fem, exemplars_rf_all_fn_fem = explainer.cluster_affinity()
filename = folder + '/' + dataset_name + '_' + clf_name + '_fn_female_clustering.txt'
stats_fn.save_clustering_results(filename, df_fn_fem, clf_name, 'female', clusters_rf_all_fn_fem, exemplars_rf_all_fn_fem)

# Random Forest + FN + Male
df_fn_male = df_fn[df_fn[gender_feat]==1]
explainer = clue.ClusterExplainer(df_fn_male.values)
clusters_rf_all_fn_male, exemplars_rf_all_fn_male = explainer.cluster_affinity()
filename = folder + '/' + dataset_name + '_' + clf_name + '_fn_male_clustering.txt'
stats_fn.save_clustering_results(filename, df_fn_male, clf_name, 'male', clusters_rf_all_fn_male, exemplars_rf_all_fn_male)

# Gradient Boosting + FP + Female
clf_name = 'gb_all'
df_fp = ml.get_fp(df_test_all, y_test_all, y_pred_gb_all)
df_fp_fem = df_fp[df_fp[gender_feat]==0]
filename = folder + '/' + dataset_name + '_' + clf_name + '_fp_samples.csv'
df_fp.to_csv(filename, index=False)

explainer = clue.ClusterExplainer(df_fp_fem.values)
clusters_gb_all_fp_fem, exemplars_gb_all_fp_fem = explainer.cluster_affinity()
filename = folder + '/' + dataset_name + '_' + clf_name + '_fp_female_clustering.txt'
stats_fp.save_clustering_results(filename, df_fp_fem, clf_name, 'female', clusters_gb_all_fp_fem, exemplars_gb_all_fp_fem)

# Gradient Boosting + FP + Male
df_fp_male = df_fp[df_fp[gender_feat]==1]
explainer = clue.ClusterExplainer(df_fp_male.values)
clusters_gb_all_fp_male, exemplars_gb_all_fp_male = explainer.cluster_affinity()
filename = folder + '/' + dataset_name + '_' + clf_name + '_fp_male_clustering.txt'
stats_fp.save_clustering_results(filename, df_fp_male, clf_name, 'male', clusters_gb_all_fp_male, exemplars_gb_all_fp_male)

# Gradient Boosting + FN + Female
df_fn = ml.get_fn(df_test_all, y_test_all, y_pred_gb_all)
df_fn_fem = df_fn[df_fn[gender_feat]==0]
filename = folder + '/' + dataset_name + '_' + clf_name + '_fn_samples.csv'
df_fn.to_csv(filename, index=False)

explainer = clue.ClusterExplainer(df_fn_fem.values)
clusters_gb_all_fn_fem, exemplars_gb_all_fn_fem = explainer.cluster_affinity()
filename = folder + '/' + dataset_name + '_' + clf_name + '_fn_female_clustering.txt'
stats_fn.save_clustering_results(filename, df_fn_fem, clf_name, 'female', clusters_gb_all_fn_fem, exemplars_gb_all_fn_fem)

# Gradient Boosting + FN + Male
df_fn_male = df_fn[df_fn[gender_feat]==1]
explainer = clue.ClusterExplainer(df_fn_male.values)
clusters_gb_all_fn_male, exemplars_gb_all_fn_male = explainer.cluster_affinity()
filename = folder + '/' + dataset_name + '_' + clf_name + '_fn_male_clustering.txt'
stats_fn.save_clustering_results(filename, df_fn_male, clf_name, 'male', clusters_gb_all_fn_male, exemplars_gb_all_fn_male)

filename = results_path + '/' + dataset_name + '/' + dataset_name + '_clustering_stats_all_fp.csv'
stats_fp.save_statistics(filename)
filename = results_path + '/' + dataset_name + '/' + dataset_name + '_clustering_stats_all_fn.csv'
...
df_fp = ml.get_fp(df_test_all, y_test_all, y_pred_rf_all)
df_fp_fem = df_fp[df_fp[gender_feat]==0]
prototypes_indexes = exemplars_rf_all_fp_fem
...
# FP + Male
df_fp_male = df_fp[df_fp[gender_feat]==1]
prototypes_indexes = exemplars_rf_all_fp_male
...
df_fn = ml.get_fn(df_test_all, y_test_all, y_pred_rf_all)
df_fn_fem = df_fn[df_fn[gender_feat]==0]
prototypes_indexes = exemplars_rf_all_fn_fem
...
# FN + Male
df_fn_male = df_fn[df_fn[gender_feat]==1]
prototypes_indexes = exemplars_rf_all_fn_male
...
df_fp = ml.get_fp(df_test_all, y_test_all, y_pred_gb_all)
df_fp_fem = df_fp[df_fp[gender_feat]==0]
prototypes_indexes = exemplars_gb_all_fp_fem
prototypes_to_explain = df_fp_fem.iloc[prototypes_indexes]
...
# FP + Male
df_fp_male = df_fp[df_fp[gender_feat]==1]
prototypes_indexes = exemplars_gb_all_fp_male
prototypes_to_explain = df_fp_male.iloc[prototypes_indexes]
...
df_fn = ml.get_fn(df_test_all, y_test_all, y_pred_gb_all)
df_fn_fem = df_fn[df_fn[gender_feat]==0]
prototypes_indexes = exemplars_gb_all_fn_fem
prototypes_to_explain = df_fn_fem.iloc[prototypes_indexes]
...
# FN + Male
df_fn_male = df_fn[df_fn[gender_feat]==1]
prototypes_indexes = exemplars_gb_all_fn_male
prototypes_to_explain = df_fn_male.iloc[prototypes_indexes]
